...
import { 
  ...
  Alert, TextInput, ScrollView, Modal, RefreshControl, Platform, SafeAreaView
} from 'react-native';
...
const CollectCanteenScreen = ({ route, navigation }) => {
  ...
  const handleStudentSelection = async (student, targetCategory, className) => {
    if (targetCategory === 'Advanced') {
      setSelectedStudentForAdvance({ student, className });
      setAdvanceAmount('');
      setShowAdvancedModal(false);
      setShowAdvanceAmountModal(true);
      return;
    }
    await performStudentMove(student, targetCategory, className);
  };
  ...
  const renderStudent = (student, category, className) => {
    ...
    return (
      <View key={student.studentID} style={[styles.studentRow, isChecked && styles.activeRow]}>
        ...
        <TouchableOpacity 
          style={{ flex: 1 }} 
          onPress={() => { if(isAdv) handleStudentSelection(student, 'Advanced', className); }}
        >
  ...
  return (
    <SafeAreaView style={styles.container}>
            ...
            <ScrollView style={styles.townList}>
              {availableTowns.map((t, i) => (
                <TouchableOpacity key={i} style={styles.townItem} onPress={() => {
                  setSelectedTown(typeof t === 'string' ? t : t.town);
      ...
      <Modal visible={!!showAdvanceAmountModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.advanceAmountModalContainer}>
            ...
            <Text style={styles.modalSub}>{selectedStudentForAdvance?.student?.name || (selectedStudentForAdvance?.student ? `${selectedStudentForAdvance.student.Fname} ${selectedStudentForAdvance.student.Lname}` : '')}</Text>
            <TextInput 
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => setShowAdvanceAmountModal(false)}>
                <Text style={styles.cancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.confirmBtn} onPress={() => performStudentMove(selectedStudentForAdvance.student, 'Advanced', selectedStudentForAdvance.className, advanceAmount)}>
      ...
      <StudentSelectionModal visible={showAdvancedModal} onClose={() => setShowAdvancedModal(false)} students={modalStudents} onSelectStudent={(s) => handleStudentSelection(s, 'Advanced', selectedClass)} title="Add to Advanced" currentGroupStudents={classData[selectedClass]?.advanced || []} />
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  classCard: { margin: 12, backgroundColor: '#fff', borderRadius: 16, padding: 12, elevation: 2, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 10 },
  ...
  categoryTitle: { fontSize: 12, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  emptyText: { fontSize: 12, color: '#94a3b8', fontStyle: 'italic', paddingLeft: 4 },
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: '#fff', padding: 16, borderTopWidth: 1, borderTopColor: '#e2e8f0', elevation: 20 },
  ...
  statLab: { fontSize: 10, color: '#64748b', textTransform: 'uppercase' },
  ...
  advanceAmountModalTitle: { fontSize: 20, fontWeight: '800', color: '#1e293b', textAlign: 'center' },
